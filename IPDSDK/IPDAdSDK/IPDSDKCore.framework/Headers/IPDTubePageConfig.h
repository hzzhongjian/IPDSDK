...
typedef NS_ENUM(NSInteger, IPDTubePageVideoProgressBarStyle) {//进度条样式
    IPDTubePageVideoProgressBarStyleLightContent     = 0,  // 浅色系
    IPDTubePageVideoProgressBarStyleDarkContent      = 1   // 深色系
};

// 需要展示的tab，默认全部展示
typedef NS_ENUM(NSInteger, IPDTubePageDrawVideoVCTabOptions) {
    IPDTubePageDrawVideoVCTabOptions_playlet       = 1 << 0, // 详情页Tab
    IPDTubePageDrawVideoVCTabOptions_playlet_feed  = 1 << 1, // 推荐Tab
    IPDTubePageDrawVideoVCTabOptions_theater       = 1 << 2, // 剧场Tab
};

@interface IPDTubePageConfig : NSObject
...
@property (nonatomic, assign) IPDTubePageVideoProgressBarStyle progressBarStyle;
/// 隐藏底部跳转区域
...
/// 需要展示的tab，默认全部展示
@property (nonatomic, assign) IPDTubePageDrawVideoVCTabOptions drawVCTabOptions;
